...
func (g *openAPIGenerator) processRoute(spec *OpenAPISpec, route RouteInfo) {
	...
	if unifiedSchema, ok := route.Metadata["openapi.requestSchema.unified"]; ok {
		...
		if components.HasBody && components.BodySchema != nil {
			operation.RequestBody = g.buildRequestBody(spec, components.BodySchema, route.Metadata)
		}
	} else {
...
func (g *openAPIGenerator) buildRequestBody(spec *OpenAPISpec, schema *Schema, metadata map[string]any) *RequestBody {
	// Get content types
