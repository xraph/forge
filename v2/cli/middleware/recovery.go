...
func NewRecoveryMiddleware(logger logger.Logger) cli.CLIMiddleware {
...
func NewRecoveryMiddlewareWithConfig(logger logger.Logger, config RecoveryConfig) cli.CLIMiddleware {
...
func (rm *RecoveryMiddleware) handlePanic(ctx cli.CLIContext, recovered interface{}) error {
	...
	return errors.ErrInternalError("command panicked", fmt.Errorf("panic: %v", recovered))
}
