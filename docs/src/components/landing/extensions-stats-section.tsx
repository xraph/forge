...
const categories = [
    {
        icon: Database,
        title: 'Data & Storage',
        extensions: ['PostgreSQL', 'MySQL', 'MongoDB', 'Redis', 'SQLite', 'ClickHouse', 'BadgerDB'],
    },
    {
        icon: Radio,
        title: 'Transport & Protocol',
        extensions: ['gRPC', 'GraphQL', 'WebSocket', 'NATS', 'Kafka', 'MQTT', 'oRPC'],
    },
    {
        icon: Lock,
        title: 'Security & Middleware',
        extensions: ['JWT Auth', 'OAuth2', 'CORS', 'Rate Limiting', 'RBAC', 'Compression', 'Recovery'],
    },
    {
        icon: Server,
        title: 'Infrastructure',
        extensions: ['Prometheus', 'OpenTelemetry', 'Consul', 'mDNS', 'Docker', 'Kubernetes', 'Gateway'],
    },
    {
        icon: Brain,
        title: 'AI & Intelligence',
        extensions: ['LLM Providers', 'ReAct Agents', 'RAG Pipelines', 'MCP Server', 'Embeddings', 'Guardrails'],
    },
    {
        icon: Zap,
        title: 'Runtime & Utilities',
        extensions: ['Cron Jobs', 'Event Bus', 'Feature Flags', 'Search', 'Streaming', 'Dashboard', 'Queue'],
    },
];
                    ...
                    <CardHeader>
                        <div className="md:p-6">
                            ...
                            <p className="text-fd-muted-foreground mt-3 max-w-sm text-sm">
                                From databases to AI, transport layers to security. Every extension
                                integrates in one line with full type safety and lifecycle management.
                            </p>
                        </div>
                    </CardHeader>
                        ...
                        <div className="relative mb-6 sm:mb-0">
                            ...
                            <div className="relative overflow-hidden rounded-r-lg border border-fd-border bg-zinc-950/80 p-4">
                                <pre className="text-[12px] leading-relaxed font-mono text-zinc-400">
                                    <div><span className="text-zinc-600">// Add any extension in one line</span></div>
                                    <div><span className="text-zinc-300">app</span>.<span className="text-sky-400">Use</span>(<span className="text-sky-400">PostgreSQL</span>.<span className="text-sky-400">Extension</span>())</div>
                                    <div><span className="text-zinc-300">app</span>.<span className="text-sky-400">Use</span>(<span className="text-sky-400">Redis</span>.<span className="text-sky-400">Extension</span>())</div>
                                    <div><span className="text-zinc-300">app</span>.<span className="text-sky-400">Use</span>(<span className="text-sky-400">GRPC</span>.<span className="text-sky-400">Extension</span>())</div>
                                    <div><span className="text-zinc-300">app</span>.<span className="text-sky-400">Use</span>(<span className="text-sky-400">AI</span>.<span className="text-sky-400">Extension</span>())</div>
                                    <div><span className="text-zinc-300">app</span>.<span className="text-sky-400">Use</span>(<span className="text-sky-400">Gateway</span>.<span className="text-sky-400">Extension</span>())</div>
                                </pre>
                            </div>
                        </div>
                    </CardContent>
                    ...
                    <div className="grid grid-cols-4 gap-3 md:grid-cols-4">
                        {extensionIcons.map((ext) => (
                            ...
                            >
                                ...
                                <span className="text-[10px] text-fd-muted-foreground">{ext.label}</span>
                            </div>
                        ))}
                    </div>
                </Card>
                ...
                <Card className="group relative shadow-black/5 sm:col-span-3 sm:rounded-none sm:rounded-br-xl border-fd-border bg-fd-card">
                    ...
                    <CardContent className="relative h-fit px-6 pb-6 md:px-8 md:pb-8">
                        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                            {categories.map((cat) => (
                                <div key={cat.title} className="border border-fd-border p-4 transition-colors hover:bg-fd-accent/20">
