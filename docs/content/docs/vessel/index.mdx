...
- **Constructor injection** -- automatic dependency resolution with `In`/`Out` structs
- **Multiple lifecycles** -- singleton, transient, and scoped services
- **Named instances** -- `WithName` and `WithAliases` for multiple instances of the same type
