...
<Callout type="tip">
For most applications, use `cli.RunApp()` for zero-boilerplate CLI wrapping with built-in serve, migrate, and extension commands. See [App Runner](/docs/cli/app-runner).
</Callout>
...
func migrateStatusHandler(ctx cli.CommandContext) error {
    ...
    if len(pending) > 0 {
        ctx.Warning("  Database migrations are pending")
...
func bulkEmailHandler(ctx cli.CommandContext) error {
    ...
    if errorCount > 0 {
        ctx.Warning(fmt.Sprintf("  %d emails failed to send", errorCount))
    } else {
...
func productionCommands(ctx cli.CommandContext) error {
    ...
    ctx.Warning("  Running in PRODUCTION environment")
