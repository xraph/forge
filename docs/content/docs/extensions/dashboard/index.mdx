...
func main() {
    ...
    // Dashboard available at http://localhost:8080/dashboard/ui
...
|---|---|---|
| GET | `/ui` | Prebuilt React shell (overview, health, metrics, services) |
| GET | `/ui/*` | Shell static assets and client-side routes |

The shell reads its data over the contract envelope (see below), not through the ForgeUI pages listed next. `/`, `/health`, `/metrics`, and `/services` are not served: nothing renders those paths and nothing redirects them to the shell.

### Contributor Pages (ForgeUI)
...
These are still served by the templ contributor system: extensions can register their own pages, widget fragments, and settings panels, and the shell embeds them through the fragment proxy. Only forge's own built-in core pages were removed.
