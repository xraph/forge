...
The dashboard itself is a prebuilt React shell, served at `{BasePath}/ui` and mounted directly by the extension (the router registers `{BasePath}/ui` plus a wildcard beneath it, both handled by the same SPA handler). The shell covers the overview, health, metrics, and services views that used to be separate server-rendered pages, and it reads its data over the contract envelope rather than rendering server-side.

A JSON API also still serves the same data directly, at `{BasePath}/api/overview`, `{BasePath}/api/health`, `{BasePath}/api/metrics`, and `{BasePath}/api/services`, for scripted or direct access outside the shell.

The templ **contributor system** described above is separate from the shell and still fully supported: extensions can contribute their own pages, widget fragments, and settings panels, which render through the fragment proxy.
