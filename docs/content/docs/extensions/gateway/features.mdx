...
1. The gateway resolves `*discovery.Service` from the DI container on startup.
2. It periodically polls (or watches) the discovery backend for services.
3. For each service, it checks the metadata for `farp.enabled=true`.
4. If FARP is enabled, it reads `farp.openapi`, `farp.asyncapi`, and `farp.graphql` from the metadata.
5. It generates routes based on the schema types:
...
6. Routes are prefixed with the service name (e.g., `/user-service/*`) if `AutoPrefix` is enabled.
