...
The service is managed by Vessel. The queue owns every connection it uses, so it depends on no
other extension.
...
- The Redis backend uses Redis Streams over a connection the queue opens itself.
