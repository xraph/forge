...
fmt.Println("Base config:", result.BaseConfigPath)
fmt.Println("Local config:", result.LocalConfigPath)
...
1. Start at the first search path (usually `.`)
2. Look for any of the `ConfigNames` files
3. If not found, walk up to the parent directory
4. Repeat up to `MaxDepth` levels
5. Once found, also look for `LocalConfigNames` in the same directory
