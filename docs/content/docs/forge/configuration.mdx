...
Forge also supports a `.forge.yaml` file for CLI and runtime configuration. This is separate from the application config and is used primarily by the `forge` CLI tool:
...
When running your app directly (not through `forge dev`), the `dev.port` setting from `.forge.yaml` is used if the HTTP address has not been explicitly configured. Priority order:

1. `PORT` environment variable
2. Explicit `WithHTTPAddress()` in code
3. `dev.port` from `.forge.yaml`
4. Default `:8080`
