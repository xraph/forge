...
- **`forge db`** (aliases: `database`) - Database management tools, built on [grove](https://github.com/xraph/grove)
  - `forge db init` - Scaffold the migrations package, then create grove's tracking tables
  - `forge db migrate` - Run pending migrations
  - `forge db rollback` - Roll back the last migration group
  - `forge db status` - Show migration status
  - `forge db create-sql` - Create a SQL migration (writes a `.up.sql`, a `.down.sql`, and a `.go` file that registers them)
  - `forge db create-go` - Create a Go migration
  - `forge db adopt` - Record a database's pre-grove migration history, once, after upgrading
  - `forge db reset` - Roll back and re-run every migration

See [Database Commands](/docs/forge/database) for the full reference, including which DSN schemes are supported and what changed if you have migrations from before grove.
