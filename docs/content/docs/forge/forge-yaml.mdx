...
database:
  ...
  disable_migrations: false              # Skip auto-migrations on serve (default: false)
...
| `disable_migrations` | `bool` | `false` | Skip auto-migrations on serve. Also settable via `forge.WithDisableMigrations()`. See [Running Apps](/docs/forge/running-apps). |
