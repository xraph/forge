...
And they need the binder to be reading the envelope the server actually sends. A channel served by `extensions/streaming` names its frames in `event`, not `type`, so its binder takes `decode: forgeStreamingDecoder()` — without it the socket connects, the frames arrive, and every one of them is discarded before it reaches the cache. See [Which field carries the name](/docs/web-client/invalidation#which-field-carries-the-name).
</Callout>
