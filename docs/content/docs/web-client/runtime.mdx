---
title: The runtime
description: The normalized entity store, tags, and the placement escape hatch
icon: Boxes
---

import { Callout } from 'fumadocs-ui/components/callout';

`@forge-go/client-core` is where everything a hook does actually lives. Generated output holds types and one-line bindings; the store, the tag graph, the query engine and the transports are here, so a runtime defect is fixed by publishing this package rather than by regenerating every repository that consumes your API.

You rarely call it directly. It is worth understanding anyway, because three of its behaviours explain almost everything surprising a normalized cache does.

## A response is split in two

On arrival, entities go into a flat store keyed `Type:id`. What is left is a **skeleton** holding references and inline scalars.

```
GET /orders  →  { items: [{id: 7, total: 99, customer: {id: "c-3", name: "Ada"}}, {id: 8, …}] }

                              normalize
                                  ↓
entity store                             query skeleton  (key: listOrders|{})
┌───────────────────────────────┐        ┌──────────────────────────────┐
│ Order:7       {total: 99, …} v4│        │ { items: [→Order:7, →Order:8] │
│ Order:8       {…}            v1│        │   total: 2 }                  │
│ Customer:c-3  {name: "Ada"}  v2│        └──────────────────────────────┘
└───────────────────────────────┘         deps: {Order:7, Order:8, Customer:c-3}
```

Reading a query rehydrates the skeleton against the store. That indirection is why a `PATCH /orders/7` updates the list, the detail page and a sidebar badge with no refetch: all three reference `Order:7`.

The runtime knows where to descend because the generated `entities` table told it:

```ts
export const entities = {
  Customer: { idField: 'id' },
  Order: { idField: 'id', fields: { customer: 'Customer' } },
  OrderPage: { fields: { items: 'Order' } },
} as const;
```

`OrderPage` has edges and no identity — a signpost the walk passes through rather than a record it stores.

### What a refetch preserves

This distinction matters the first time you profile a render:

- **Entity identity is preserved.** A response re-delivering an unchanged `Order:7` writes no new version, so the rehydrated `Order:7` is the same object it was. A `memo`'d row rendering it skips. The same holds for an unchanged `Customer` subtree beneath it.
- **Container identity is not.** A fresh response is a fresh skeleton — new arrays, new wrapper objects — so the list *containing* those orders is a new array.

The store declines to claim it knows a list's shape is unchanged merely because the entities in it are. The property still does its job, because what a component renders per row is the entity, not the array.

<Callout type="info">
A write whose data is deep-equal to what is stored is not a write: the version does not move and no identity downstream changes. A poll returning the same bytes re-renders nothing.
</Callout>

## Tags decide what refetches

A mounted query carries two kinds of tag in one set: the `provides` from the manifest resolved against its arguments, and the entity keys `normalize` reported for its skeleton. Those keys are already spelled `Type:id` — exactly what a mutation to `Order:7` invalidates — so a list that loaded `Order:7` is reachable from a write to it with no second index.

When a mutation settles, the runtime intersects its `invalidates` against the tag index, marks matches stale, and refetches **mounted queries only**, coalesced into one batch per tick. Unmounted queries stay stale and refetch on next mount. Refetching data nobody is looking at turns a smart cache into a bandwidth complaint.

That is the whole of the difference between the two tag shapes:

- **`Order:7`** reaches every query that has *seen* order 7. A field update needs no refetch at all — the record is patched in place and dependents re-render.
- **`Order[]`** reaches queries over the collection. Membership changes need the network, because the server decides who is in a filtered list.

<Callout type="warn">
A tag template that resolves to nothing is skipped. `resolveTag` returns `undefined` rather than a partially substituted string, because a tag that quietly becomes `Customer:` matches no query, fires nothing and reports nothing. It is surfaced as `cause.unresolved` in [devtools](/docs/web-client/devtools) — the only place it is visible.
</Callout>

## The placement escape hatch

Refetching after a create is correct and often wasteful: you already have the created order, and it belongs at the top of the list you are looking at. Placement lets the call site say so.

```ts
const create = useMutation(useCreateOrder, {
  place: {
    'Order[]': (order, current, args) =>
      args.query?.status && args.query.status !== order.status
        ? undefined // filtered list this order does not belong to: refetch
        : [order, ...current],
  },
});
```

Returning a list skips that query's refetch. **Returning `undefined` means "I don't know" and falls back to refetching.**

That fallback is the point. The runtime never reasons about whether an entity belongs in a filtered or paginated window — that is the Relay connection-directive tarpit — and the application is allowed to decline for the cases it cannot decide. A filtered list whose predicate the callback cannot evaluate returns `undefined` and gets correct data at the cost of one request.

Two rules worth knowing:

- **All or nothing per query.** A query matched by both `Order[]` and `Customer:3` where only the first has a callback still refetches. Placing one while the other is unhandled leaves the query looking updated while being wrong.
- **A callback that throws** is reported through `onError` and treated as `undefined`. It does not take the rest of the batch with it.

## Retries, auth and identity

- **Retries apply to idempotent methods only** by default — `GET`, `HEAD`, `PUT`, `DELETE` — with exponential backoff and jitter, and no retry on 4xx except 408 and 429. Retrying a `POST` on a timeout produces duplicate orders.
- **A 401 triggers a single-flight refresh** with one retry. Concurrent requests queue behind the in-flight refresh rather than stampeding.
- **The store is partitioned by principal and dropped on identity change.** A normalized store keys `Order:7` globally with no memory of who fetched it; without partitioning, entities from one session stay addressable in the next. This is a correctness property, and it is a class of defect a response-keyed cache does not have.

## Live frames take the same path

A socket frame is a mutation the client did not initiate. It decodes, matches its binding from the `streams` table, and applies the declared intent plus any tag invalidation through **the same code path** a mutation response takes — one implementation, not two.

Two properties that are cheap now and expensive later:

- **Gap recovery.** A dropped socket means missed frames, and a reconnected client looks correct while being wrong. On reconnect the runtime invalidates every tag bound to that channel and refetches mounted live queries. Without it, staleness after a closed laptop lid presents as "it just stops updating" and is unfalsifiable from the outside.
- **Write batching.** Frames coalesce into one store commit per animation frame. A channel at 200 msg/s must not mean 200 renders.

The decode step is injected rather than assumed, because the envelope is the server's business and Forge has two of them: a plain WebSocket handler names the frame in `type`, while `extensions/streaming` names it in `event` and keeps `type` for the transport kind. The default reads `event` first, so a streaming-extension channel's domain frames bind with no configuration. A streaming-extension channel should still pass `forgeStreamingDecoder` as the binder's `decode`: it knows the seven reserved transport kinds and drops those frames silently, where the default reports each one as an unknown message on a channel that is working exactly as designed, and it owns the `channel_id` mapping for a socket carrying more than one channel. See [Which field carries the name](/docs/web-client/invalidation#which-field-carries-the-name).

## Time-based freshness

Tags are one reason a query refetches. Time is the other, and it sits underneath the same cache with no separate wiring to learn.

### A cache-wide default

`QueryCache` takes a `staleTime` in milliseconds. Leave it unset and it is `Infinity`, which means a query is fresh until a tag says otherwise, exactly as it always has been:

```ts
configureClient({
  transport: new RestTransport({ client }),
  entities,
  staleTime: 30_000,
});
```

Thirty seconds after a query settles, mounting it again triggers a background refetch instead of serving the cached value untouched. Inside that window, mounting it again is free.

### `WithStaleTime` on a route

One endpoint can carry a shorter default than the rest of the API:

```go
r.GET("/orders", listOrders,
    forge.WithOperationID("listOrders"),
    forge.WithResponseSchema(200, "OrderPage", &OrderPage{}),
    forge.WithStaleTime(15*time.Second),
)
```

Only a `GET` or a `HEAD` can carry this. Declare it on a write and the generator warns and drops it, because a write's response is not a cached result. A duration that truncates to zero or below is dropped the same way, silently: zero would mean "stale the instant it lands," which is not what `500*time.Microsecond` was trying to say.

### A per-call override

The call site beats both of the above:

```tsx
const { data } = useQuery(useListOrders, undefined, { staleTime: 5_000 });
```

Every framework adapter exposes the same option, under the same name, for the same reason: it is the one knob that can only make a query refresh more eagerly, never less.

### Installing the triggers

`revalidateOnFocus` and `revalidateOnReconnect` both call `cache.revalidate()`, which refetches every mounted query that has actually gone stale and leaves the rest alone. `poll` does not: it calls `cache.refetch()` on its own interval, unconditionally, whether or not the query has gone stale and whether or not anyone is even watching it beyond the poll itself. Set `staleTime: 600_000` and poll every second, and you still get a request every second. If you use both on the same query, the shorter of the two wins: `poll`'s interval, not `staleTime`, decides how often the network gets hit.

```ts
import { getClient, revalidateOnFocus, revalidateOnReconnect, poll } from '@forge-go/client-core';
import { ops } from './generated/ops';

const cache = getClient();

revalidateOnFocus(cache);
revalidateOnReconnect(cache);
poll(cache, ops.listOrders, undefined, 10_000);
```

`revalidateOnFocus` listens for `visibilitychange`, not `focus`, on purpose: returning to a tab that never stopped being visible should not refetch data you were already looking at. `revalidateOnReconnect` listens for the browser's `online` event. `poll` refetches one query on an interval and pauses while the tab is hidden unless you pass `whileHidden: true`. All three return a disposer; call it on teardown to stop listening.

### Precedence, and the strictest subscriber

A `staleTime` resolves in one order: the call site, then the operation's own `WithStaleTime`, then the cache's default, then `Infinity` if none of those said anything. That last case is the one that matters most: a client where nobody called `WithStaleTime`, passed a per-call `staleTime`, or set a cache default has time-based freshness turned off entirely, and behaves exactly as it did before this feature existed.

Two components can mount the same query with two different values. `QueryCache` tracks a `staleTime` per live subscriber and takes the minimum across all of them for as long as more than one is mounted, so the stricter caller always wins. A dashboard tile asking for five-second freshness pulls the whole query down to five seconds even while a list on the same screen is content with sixty.

`staleTime` is also its own throttle, which is why `revalidateOnFocus` and `revalidateOnReconnect` need none of their own. Alt-tabbing through ten background tabs calls `revalidate()` ten times, but a query that just refetched has not gone stale yet, so nine of those ten calls find nothing to do. A focus storm cannot become a request storm, for a query that last settled successfully.

An errored query is the exception, and deliberately so. A failed fetch does not stamp a new settle time, so a query that broke stays exactly as expired as it was the moment it broke, and every focus or reconnect after that retries it. That is not a leak in the throttle. Retrying the moment you come back to a tab is what you want from a query sitting on a stale error, and protecting it from a retry would mean lying about how fresh a failed result is.

### Where staleTime doesn't reach

Vue and Angular take `staleTime` as a plain number, not a reactive one. In both adapters the neighbouring `live` option is reactive, so toggling it takes effect immediately, but `staleTime` is read once and only re-read when the query's own arguments change, because re-subscription watches the cache key, not the options object. Change `staleTime` on its own and nothing happens until the arguments move too; the workaround is to change both together. React does not have this limitation: `staleTime` sits in the same dependency list as the operation and the key, so a changed value re-subscribes on its own.

A hydrated query is stamped fresh at the moment it hydrates, not at the moment the server produced it. `restore()` sets the settle time to the hydration clock, so a page that sat behind a CDN for ten minutes paints as though its data had just arrived, and stays fresh for a further `staleTime` after that. Pass `hydrate(cache, state, { stale: true })` (or `stale` on `HydrationBoundary`) when that gap matters: it marks the query so the next mount refetches instead of trusting the hydration timestamp.

A live query's channel does not buy it any slack from the clock either. A stream frame updates the entities it names and invalidates whatever tags the binding declares, but that is not the same thing a response does when it settles a query, so a frame never moves `settledTime`. A live query with a finite `staleTime` keeps refetching on schedule regardless, even while the socket underneath it is current to the second.

## Using the pieces directly

Each layer is usable on its own, and the transports are injected rather than reached for — nothing here touches the network by itself.

```ts
import { EntityStore, denormalize } from '@forge-go/client-core';
import { entities, ops } from './generated/ops';

const store = new EntityStore();
const { skeleton, deps } = store.write(response, entities, ops.listOrders.rootType);

denormalize(skeleton, store); // the response, rebuilt from the store
```

| Export | What it does |
|---|---|
| `normalize(value, entities, rootType?)` | Pure. Returns `{skeleton, records, deps}` |
| `EntityStore#write(...)` | Normalize and commit |
| `denormalize(skeleton, store)` | Rebuild, with structural sharing |
| `QueryRegistry` | Mounted queries and their tags |
| `Invalidator` | Turns a settled mutation into a refetch batch |
| `QueryCache` | The query engine |
| `RestTransport` | Drives the generated REST client |
| `StreamBinder`, `SubscriptionManager` | Live frames and ref-counted sockets |
| `decodeFrame`, `forgeStreamingDecoder` | The two envelopes a frame arrives in |

<Callout type="warn">
**Nothing here is copied defensively.** Values handed to the store are not cloned, and values read back are the store's own objects. Mutating one in place moves the cache underneath every query referencing it, with no version bump and no re-render.
</Callout>

## Known gaps

Documented by the runtime itself, not inferred:

- **Optimistic overlays are not implemented.** The layered design exists; the code does not.
- **SSR revival.** A skeleton serializes, but a deserialized one is not recognised as a skeleton, so `dehydrate`/`hydrate` are not offered rather than offered half-working.
- **Entity garbage collection.** The cache caps *queries*, but an entity no live skeleton references is still held. `EntityStore#evict` exists; a policy driving it does not.
- **Field renaming does not reach the hook path.** The transport drives the HTTP client below the generated per-endpoint methods that set the codecs. Under the default `camel` naming this means a hook can return wire-cased fields while the direct REST client returns renamed ones from the same package. With `--field-naming preserve` and no overrides, no codec table is emitted and the two are exactly equivalent.
- **WebTransport binding** is unwritten, though the subscription manager accepts any connection.

See [Not yet shipped](/docs/web-client/not-yet-shipped) for the rest.
