...
Producing one package with both from Forge's two documents is not reachable through the CLI today. Track it under [Not yet shipped](/docs/web-client/not-yet-shipped).
</Callout>
...
<Callout type="warn">
**A channel served by `extensions/streaming` needs `forgeStreamingDecoder`**, passed as the binder's `decode`. Without it `type` wins, every frame on the channel decodes as `message`, no row in the `streams` table is keyed on `message`, and all of them are discarded as unknown messages — a socket that is open, a server that is sending, and a UI that never updates.
