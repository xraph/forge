...
The fix is to return a named page type. Generation reads through the wrapper to the element inside it:
...
<Callout type="info">
The wrapper has to hold exactly one array of one entity, and the operation has to be a read. That is enough to tag a page of orders. It is not enough to tell that page apart from a report over the same orders, so a report gets the collection tag as well, and you drop it with `forge.WithoutEntity()` on the route. For the wrappers this cannot resolve, a `{data: Order}` around one record or a response carrying two collections, declare `ForgeEnvelope`. See [Entities and normalization](/docs/web-client/entities#envelopes-how-a-list-gets-its-tags).
</Callout>
