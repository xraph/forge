...
<Callout type="warn">
`@forge-go/client-core` and the framework adapters are **not on npm yet**, so the two snippets above do not install today. The generated `rest.ts` has no such dependency and is usable now. See [Installation](/docs/web-client/installation) for what works and what does not.
</Callout>
