...
func (pd *PluginDownloader) Download(ctx context.Context, source plugins.PluginSource, progressCallback func(DownloadProgress)) (plugins.PluginPackage, error) {
	pd.mu.RLock()
	defer pd.mu.RUnlock()

	if !pd.initialized {
		return plugins.PluginPackage{}, fmt.Errorf("plugin downloader not initialized")
	}
	...
	if err != nil {
		...
		return plugins.PluginPackage{}, err
	}
...
func (pd *PluginDownloader) downloadFromSource(ctx context.Context, source plugins.PluginSource, progressCallback func(DownloadProgress)) (plugins.PluginPackage, error) {
	switch source.Type {
	case plugins.PluginSourceTypeURL:
		return pd.downloadFromURL(ctx, source.Location, source.Version, progressCallback)
	case plugins.PluginSourceTypeFile:
		return pd.downloadFromFile(ctx, source.Location, source.Version)
	case plugins.PluginSourceTypeMarketplace:
		return pd.downloadFromMarketplace(ctx, source.Location, source.Version, progressCallback)
	default:
		return plugins.PluginPackage{}, fmt.Errorf("unsupported source type: %s", source.Type)
	}
}

// downloadFromURL downloads plugin from URL
func (pd *PluginDownloader) downloadFromURL(ctx context.Context, url, version string, progressCallback func(DownloadProgress)) (plugins.PluginPackage, error) {
	req, err := http.NewRequestWithContext(ctx, "GET", url, nil)
	if err != nil {
		return plugins.PluginPackage{}, fmt.Errorf("failed to create request: %w", err)
	}

	resp, err := pd.httpClient.Do(req)
	if err != nil {
		return plugins.PluginPackage{}, fmt.Errorf("failed to download: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return plugins.PluginPackage{}, fmt.Errorf("download failed with status: %d", resp.StatusCode)
	}

	// Get content length for progress tracking
	contentLength := resp.ContentLength
	if contentLength > pd.config.MaxDownloadSize {
		return plugins.PluginPackage{}, fmt.Errorf("download size exceeds limit: %d > %d", contentLength, pd.config.MaxDownloadSize)
	}
	...
	if err != nil {
		return plugins.PluginPackage{}, fmt.Errorf("failed to read response: %w", err)
	}
...
func (pd *PluginDownloader) downloadFromFile(ctx context.Context, filePath, version string) (plugins.PluginPackage, error) {
	data, err := os.ReadFile(filePath)
	if err != nil {
		return plugins.PluginPackage{}, fmt.Errorf("failed to read file: %w", err)
	}
...
func (pd *PluginDownloader) downloadFromMarketplace(ctx context.Context, pluginID, version string, progressCallback func(DownloadProgress)) (plugins.PluginPackage, error) {
	...
	resp, err := pd.httpClient.Do(req)
	if err != nil {
		return plugins.PluginPackage{}, fmt.Errorf("failed to download from marketplace: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return plugins.PluginPackage{}, fmt.Errorf("marketplace download failed with status: %d", resp.StatusCode)
	}
...
func (pd *PluginDownloader) getCachedPackage(pluginID, version string) (plugins.PluginPackage, bool) {
	pd.cache.mu.RLock()
	...
	if !exists {
		return plugins.PluginPackage{}, false
	}
...
func (pd *PluginDownloader) cachePackage(pluginID, version string, pkg plugins.PluginPackage) error {
	pd.cache.mu.Lock()
