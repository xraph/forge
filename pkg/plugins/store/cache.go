...
type CachedPluginPackage struct {
	Package     plugins.PluginPackage `json:"package"`
	CachedAt    time.Time             `json:"cached_at"`
	ExpiresAt   time.Time             `json:"expires_at"`
...
func (pc *PluginCache) GetPluginInfo(pluginID string) (*plugins.PluginInfo, bool) {
	pc.mu.RLock()
	defer pc.mu.RUnlock()
...
func (pc *PluginCache) SetPluginInfo(pluginID string, info *plugins.PluginInfo) {
	pc.mu.Lock()
	defer pc.mu.Unlock()
...
func (pc *PluginCache) GetPackage(pluginID, version string) (plugins.PluginPackage, bool) {
	pc.mu.RLock()
	defer pc.mu.RUnlock()
...
func (pc *PluginCache) SetPackage(pluginID, version string, pkg plugins.PluginPackage) error {
	pc.mu.Lock()
	defer pc.mu.Unlock()
