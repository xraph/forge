...
type CachedSearch struct {
	...
	Results   []plugins.PluginInfo `json:"results"`
...
func (mc *MarketplaceClient) Search(ctx context.Context, query PluginQuery) ([]plugins.PluginInfo, error) {
	mc.mu.RLock()
	defer mc.mu.RUnlock()
...
func (mc *MarketplaceClient) GetPluginInfo(ctx context.Context, pluginID string) (*plugins.PluginInfo, error) {
	mc.mu.RLock()
	defer mc.mu.RUnlock()
...
func (mc *MarketplaceClient) Download(ctx context.Context, pluginID, version string) (plugins.PluginPackage, error) {
	mc.mu.RLock()
	defer mc.mu.RUnlock()

	if !mc.initialized {
		return plugins.PluginPackage{}, fmt.Errorf("marketplace client not initialized")
	}

	// Rate limit check
	if !mc.rateLimiter.Allow("download") {
		return plugins.PluginPackage{}, fmt.Errorf("rate limit exceeded")
	}
	...
	if err != nil {
		return plugins.PluginPackage{}, fmt.Errorf("failed to create download request: %w", err)
	}
	...
	if err != nil {
		...
		return plugins.PluginPackage{}, fmt.Errorf("download request failed: %w", err)
	}
	defer resp.Body.Close()

	if resp.StatusCode == http.StatusNotFound {
		return plugins.PluginPackage{}, fmt.Errorf("plugin version not found: %s@%s", pluginID, version)
	}
	...
	if resp.StatusCode != http.StatusOK {
		...
		return plugins.PluginPackage{}, fmt.Errorf("download request failed with status: %d", resp.StatusCode)
	}

	// Parse response as plugin package
	var marketplaceResp MarketplaceResponse
	if err := json.NewDecoder(resp.Body).Decode(&marketplaceResp); err != nil {
		return plugins.PluginPackage{}, fmt.Errorf("failed to decode download response: %w", err)
	}

	if !marketplaceResp.Success {
		return plugins.PluginPackage{}, fmt.Errorf("download failed: %s", marketplaceResp.Error)
	}

	// Parse plugin package
	packageData, err := json.Marshal(marketplaceResp.Data)
	if err != nil {
		return plugins.PluginPackage{}, fmt.Errorf("failed to marshal package data: %w", err)
	}

	var pkg plugins.PluginPackage
	if err := json.Unmarshal(packageData, &pkg); err != nil {
		return plugins.PluginPackage{}, fmt.Errorf("failed to unmarshal plugin package: %w", err)
	}
...
func (mc *MarketplaceClient) Publish(ctx context.Context, pkg plugins.PluginPackage) error {
	mc.mu.RLock()
	defer mc.mu.RUnlock()
