...
type PluginStore interface {
	...
	Download(ctx context.Context, pluginID, version string) (plugins.PluginPackage, error)
	Publish(ctx context.Context, plugin plugins.PluginPackage) error
...
func (ps *PluginStoreImpl) Get(ctx context.Context, pluginID string) (*plugins.PluginInfo, error) {
	ps.mu.RLock()
	defer ps.mu.RUnlock()
...
func (ps *PluginStoreImpl) Download(ctx context.Context, pluginID, version string) (plugins.PluginPackage, error) {
	ps.mu.RLock()
	defer ps.mu.RUnlock()

	if !ps.initialized {
		return plugins.PluginPackage{}, fmt.Errorf("plugin store not initialized")
	}
	...
	if ps.config.EnableMarketplace && ps.marketplace != nil {
		...
		if err == nil {
			...
			if err := ps.validator.ValidatePackage(ctx, pkg); err != nil {
				...
				return plugins.PluginPackage{}, fmt.Errorf("package validation failed: %w", err)
			}
	...
	return plugins.PluginPackage{}, fmt.Errorf("failed to download plugin %s version %s", pluginID, version)
}

// Publish publishes a plugin package
func (ps *PluginStoreImpl) Publish(ctx context.Context, plugin plugins.PluginPackage) error {
	ps.mu.RLock()
	defer ps.mu.RUnlock()
...
type PluginQuery struct {
	...
	Type       plugins.PluginType     `json:"type,omitempty"`
