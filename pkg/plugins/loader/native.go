...
type NativeLoader struct {
	...
	pluginCache map[string]plugins.Plugin
}
...
type NativePluginWrapper struct {
	...
	pluginImpl   plugins.Plugin
	capabilities []plugins.PluginCapability
	dependencies []plugins.PluginDependency
	config       interface{}
...
func NewNativeLoader(logger common.Logger, metrics common.Metrics) Loader {
	return &NativeLoader{
		...
		pluginCache: make(map[string]plugins.Plugin),
	}
}
...
func (nl *NativeLoader) LoadPlugin(ctx context.Context, source plugins.PluginSource) (plugins.Plugin, error) {
	startTime := time.Now()
...
func (nl *NativeLoader) ValidateSource(source plugins.PluginSource) error {
	if source.Location == "" {
...
func (nl *NativeLoader) GetMetadata(source plugins.PluginSource) (*plugins.PluginInfo, error) {
	soPath, err := nl.resolveSOPath(source)
	if err != nil {
		return nil, fmt.Errorf("failed to resolve SO path: %w", err)
	}
...
func (nl *NativeLoader) resolveSOPath(source plugins.PluginSource) (string, error) {
	switch source.Type {
...
func (nl *NativeLoader) extractPluginFromSO(goPlugin *plugin.Plugin, soPath string) (plugins.Plugin, error) {
	// Strategy 1: Look for a Plugin variable
	if symbol, err := goPlugin.Lookup("Plugin"); err == nil {
		if pluginImpl, ok := symbol.(plugins.Plugin); ok {
			return pluginImpl, nil
		}
		if pluginPtr, ok := symbol.(*plugins.Plugin); ok && pluginPtr != nil {
	...
	if symbol, err := goPlugin.Lookup("NewPlugin"); err == nil {
		if factory, ok := symbol.(func() plugins.Plugin); ok {
			return factory(), nil
		}
		if factoryPtr, ok := symbol.(*func() plugins.Plugin); ok && factoryPtr != nil {
...
func (nl *NativeLoader) loadNativeMetadata(soPath string) (*plugins.PluginInfo, error) {
...
func (nl *NativeLoader) loadMetadataFromJSON(path string) (*plugins.PluginInfo, error) {
...
func (nl *NativeLoader) extractMetadataFromSO(soPath string) (*plugins.PluginInfo, error) {
	goPlugin, err := plugin.Open(soPath)
	if err != nil {
		return nil, err
	}
	...
	if symbol, err := goPlugin.Lookup("PluginInfo"); err == nil {
		if info, ok := symbol.(*plugins.PluginInfo); ok {
			return info, nil
		}
		if info, ok := symbol.(plugins.PluginInfo); ok {
			return &info, nil
	...
	if symbol, err := goPlugin.Lookup("GetPluginInfo"); err == nil {
		if infoFunc, ok := symbol.(func() *plugins.PluginInfo); ok {
			return infoFunc(), nil
		}
	}
...
func (nl *NativeLoader) createDefaultMetadata(soPath string) *plugins.PluginInfo {
	...
	return &plugins.PluginInfo{
		...
		Name:        nameWithoutExt,
		Version:     "1.0.0",
		Type:        plugins.PluginTypeUtility,
...
func (npw *NativePluginWrapper) Type() plugins.PluginType                 { return npw.pluginType }
func (npw *NativePluginWrapper) Capabilities() []plugins.PluginCapability { return npw.capabilities }
func (npw *NativePluginWrapper) Dependencies() []plugins.PluginDependency { return npw.dependencies }
...
func (npw *NativePluginWrapper) GetMetrics() plugins.PluginMetrics {
...
func (npw *NativePluginWrapper) GetNativePlugin() plugins.Plugin {
