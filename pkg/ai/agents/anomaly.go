...
func NewAnomalyDetectionAgent(id, name string) ai.AIAgent {
	...
	agent := &AnomalyDetectionAgent{
		BaseAgent:           ai.NewBaseAgent(id, name, ai.AgentTypeAnomalyDetection, capabilities),
...
func (a *AnomalyDetectionAgent) Process(ctx context.Context, input ai.AgentInput) (ai.AgentOutput, error) {
...
func (a *AnomalyDetectionAgent) processAnomalyDetection(ctx context.Context, input ai.AgentInput) (ai.AgentOutput, error) {
	// Parse input
	...
	if len(dataPoints) == 0 {
		return ai.AgentOutput{}, fmt.Errorf("no data points provided")
	}
...
func (a *AnomalyDetectionAgent) processBaselineModeling(ctx context.Context, input ai.AgentInput) (ai.AgentOutput, error) {
	// Parse input
	data, ok := input.Data.(map[string]interface{})
	if !ok {
		return ai.AgentOutput{}, fmt.Errorf("invalid input data format")
...
func (a *AnomalyDetectionAgent) processAnomalyAnalysis(ctx context.Context, input ai.AgentInput) (ai.AgentOutput, error) {
	// Parse input
	data, ok := input.Data.(map[string]interface{})
	if !ok {
		return ai.AgentOutput{}, fmt.Errorf("invalid input data format")
	...
	if err != nil {
		return ai.AgentOutput{}, fmt.Errorf("invalid time range: %w", err)
	}
	...
	return ai.AgentOutput{
		...
		Actions:     []ai.AgentAction{}, // Analysis typically doesn't generate actions
		Metadata: map[string]interface{}{
