...
func NewEventBusHealthCheckFactory(container common.Container) *EventBusHealthCheckFactory {
...
func (factory *EventBusHealthCheckFactory) CreateEventBusHealthCheck(name string, brokerType string, critical bool) (health.HealthCheck, error) {
	...
	if factory.container != nil {
		if eventBus, err := factory.container.ResolveNamed(name); err == nil {
...
func RegisterEventBusHealthChecks(healthService health.HealthService, container common.Container) error {
	...
	for _, eb := range eventBuses {
		...
		if err := healthService.RegisterCheck(eb.name, check); err != nil {
