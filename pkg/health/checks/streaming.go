...
func NewStreamingHealthCheckFactory(container common.Container) *StreamingHealthCheckFactory {
...
func (factory *StreamingHealthCheckFactory) CreateStreamingHealthCheck(name string, checkType string, critical bool) (health.HealthCheck, error) {
	...
	if factory.container != nil {
		if streamingManager, err := factory.container.ResolveNamed("streaming-manager"); err == nil {
...
func RegisterStreamingHealthChecks(healthService health.HealthService, container common.Container) error {
	...
	for _, sc := range streamingChecks {
		...
		if err := healthService.RegisterCheck(sc.name, check); err != nil {
