...
type TestHealthService struct {
	...
	configuration *health.HealthServiceConfig
	mu            sync.RWMutex
...
func NewTestHealthService(name string) *TestHealthService {
	return &TestHealthService{
		...
		configuration: &health.HealthServiceConfig{
			HealthChecker: health.DefaultHealthCheckerConfig(),
		},
	}
}
...
func (ths *TestHealthService) RegisterCheck(name string, check health.HealthCheck) error {
	ths.mu.Lock()
	defer ths.mu.Unlock()
	ths.recordMethodCall("RegisterCheck")

	if err := ths.errors["RegisterCheck"]; err != nil {
		return err
	}
...
func (ths *TestHealthService) UnregisterCheck(name string) error {
	ths.mu.Lock()
	defer ths.mu.Unlock()
	ths.recordMethodCall("UnregisterCheck")

	if err := ths.errors["UnregisterCheck"]; err != nil {
		return err
	}
...
func (ths *TestHealthService) Checker() *health.HealthChecker {
...
func (ths *TestHealthService) GetConfiguration() *health.HealthServiceConfig {
	ths.mu.Lock()
	defer ths.mu.Unlock()
...
func NewTestHealthChecker() *TestHealthChecker {
	...
	return &TestHealthChecker{
		HealthChecker: health.NewHealthChecker(config, logger.NewNoopLogger(), &testMetrics{}, nil),
