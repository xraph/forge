...
import (
	...
	configcore "github.com/xraph/forge/pkg/config/core"
	"github.com/xraph/forge/pkg/config/formats"
...
func NewFileSource(path string, options FileSourceOptions) (configcore.ConfigSource, error) {
	if path == "" {
		return nil, common.ErrConfigError("file path cannot be empty", nil)
	}
	...
	if _, err := os.Stat(path); err != nil {
		if options.RequireFile || !os.IsNotExist(err) {
			return nil, common.ErrConfigError(fmt.Sprintf("configuration file not found: %s", path), err)
		}
	}
	...
	if err != nil {
		return nil, common.ErrConfigError(fmt.Sprintf("unsupported format: %s", format), err)
	}
...
func (fs *FileSource) Load(ctx context.Context) (map[string]interface{}, error) {
	...
	if err != nil {
		...
		return nil, common.ErrConfigError(fmt.Sprintf("failed to stat file %s", path), err)
	}
	...
	if err != nil {
		return nil, common.ErrConfigError(fmt.Sprintf("failed to read file %s", path), err)
	}
	...
	if err != nil {
		return nil, common.ErrConfigError(fmt.Sprintf("failed to parse file %s", path), err)
	}
	...
	if fs.options.ExpandSecrets {
		...
		if err != nil {
			return nil, common.ErrConfigError(fmt.Sprintf("failed to expand secrets in %s", path), err)
		}
	}
	...
	if validator, ok := fs.processor.(interface {
		...
		if err := validator.Validate(data); err != nil {
			return nil, common.ErrConfigError(fmt.Sprintf("validation failed for file %s", path), err)
		}
	}
...
func (fs *FileSource) Watch(ctx context.Context, callback func(map[string]interface{})) error {
	...
	if fs.watching {
		return common.ErrConfigError("already watching file", nil)
	}

	if !fs.IsWatchable() {
		return common.ErrConfigError("file watching is not enabled", nil)
	}

	watcher, err := fsnotify.NewWatcher()
	if err != nil {
		return common.ErrConfigError("failed to create file watcher", err)
	}
	...
	if err := watcher.Add(dir); err != nil {
		watcher.Close()
		return common.ErrConfigError(fmt.Sprintf("failed to watch directory %s", dir), err)
	}
...
func (fs *FileSource) GetSecret(ctx context.Context, key string) (string, error) {
	...
	value := os.Getenv(key)
	if value == "" {
		return "", common.ErrConfigError(fmt.Sprintf("secret not found: %s", key), nil)
	}
	return value, nil
}
...
func (fs *FileSource) handleWatchError(err error) {
	...
	if fs.errorHandler != nil {
		fs.errorHandler.HandleError(nil, common.ErrConfigError(fmt.Sprintf("file watch error for %s", fs.path), err))
	}
}
...
func NewFileSourceFactory(logger common.Logger, errorHandler common.ErrorHandler) *FileSourceFactory {
