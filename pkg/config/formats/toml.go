...
func (tp *TOMLProcessor) Parse(data []byte) (map[string]interface{}, error) {
	...
	if _, err := decoder.Decode(&result); err != nil {
		return nil, common.ErrConfigError(fmt.Sprintf("failed to parse TOML: %v", err), err)
	}

	// Convert the result to ensure proper types
	converted, err := tp.convertValue(result)
	if err != nil {
		return nil, common.ErrConfigError(fmt.Sprintf("failed to convert TOML values: %v", err), err)
	}

	// In strict mode, perform additional validation after parsing
	if tp.strictMode {
		if err := tp.validateStrictMode(result); err != nil {
			return nil, common.ErrConfigError(fmt.Sprintf("strict mode validation failed: %v", err), err)
		}
	}
	...
	return nil, common.ErrConfigError("TOML root must be an object", nil)
}
...
func (tp *TOMLProcessor) validateValue(path string, value interface{}) error {
	...
	default:
		...
		if tp.strictMode && !tp.allowUnknown {
			return common.ErrValidationError(path, fmt.Errorf("unsupported value type: %T", value))
		}
...
func (tp *TOMLProcessor) ParseToStruct(data []byte, target interface{}) error {
	...
	if _, err := decoder.Decode(target); err != nil {
		return common.ErrConfigError(fmt.Sprintf("failed to decode TOML into struct: %v", err), err)
	}
...
func (tp *TOMLProcessor) FormatData(data map[string]interface{}) ([]byte, error) {
	...
	if err != nil {
		return nil, common.ErrConfigError(fmt.Sprintf("failed to prepare data for TOML: %v", err), err)
	}
	...
	if err := encoder.Encode(converted); err != nil {
		return nil, common.ErrConfigError(fmt.Sprintf("failed to encode TOML: %v", err), err)
	}
...
func (tp *TOMLProcessor) ValidateStructTags(target interface{}) error {
	...
	if targetType.Kind() != reflect.Struct {
		return common.ErrConfigError("target must be a struct", nil)
	}
...
func (tp *TOMLProcessor) validateTOMLTag(tag, fieldPath string) error {
	...
	if name != "" && !tp.isValidTOMLFieldName(name) {
		return common.ErrValidationError(fieldPath, fmt.Errorf("invalid TOML field name: %s", name))
	}
	...
	for i := 1; i < len(parts); i++ {
		...
		if !tp.isValidTOMLOption(option) {
			return common.ErrValidationError(fieldPath, fmt.Errorf("invalid TOML tag option: %s", option))
		}
	}
...
func GetProcessor(logger common.Logger) FormatProcessor {
...
func GetStrictProcessor(logger common.Logger) FormatProcessor {
	return NewTOMLProcessor(TOMLProcessorOptions{
		Logger:       logger,
		StrictMode:   true,
		AllowUnknown: false,
	...
	case map[string]interface{}:
		for key, val := range v {
			...
			if !tv.processor.isValidTOMLFieldName(key) {
				return common.ErrValidationError(keyPath, fmt.Errorf("invalid TOML table key: %s", key))
			}
