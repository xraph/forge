...
type CustomCollector struct {
	...
	collectors       map[string]CustomMetricCollector
...
type CustomCollectorConfig struct {
	...
	EnableBusinessMetrics    bool          `yaml:"enable_business_metrics" json:"enable_business_metrics"`
	...
	EnableFeatureMetrics     bool          `yaml:"enable_feature_metrics" json:"enable_feature_metrics"`
	MaxCustomMetrics         int           `yaml:"max_custom_metrics" json:"max_custom_metrics"`
	CallbackTimeout          time.Duration `yaml:"callback_timeout" json:"callback_timeout"`
}

// CustomMetricCollector defines interface for custom metric collectors
...
type CustomCallback func() (map[string]interface{}, error)
...
func NewCustomCollector(logger logger.Logger) metrics.CustomCollector {
...
func NewCustomCollectorWithConfig(config *CustomCollectorConfig, logger logger.Logger) metrics.CustomCollector {
	return &CustomCollector{
		...
		enabled:          true,
		collectors:       make(map[string]CustomMetricCollector),
...
func (cc *CustomCollector) Collect() map[string]interface{} {
	cc.mu.Lock()
	defer cc.mu.Unlock()
	...
	cc.metrics = make(map[string]interface{})
...
func (cc *CustomCollector) RegisterCustomCollector(collector CustomMetricCollector) error {
	cc.mu.Lock()
	defer cc.mu.Unlock()
...
func (cc *CustomCollector) RecordBusinessMetric(name string, value interface{}, metricType, unit, description string, tags map[string]string) {
	cc.mu.Lock()
	defer cc.mu.Unlock()
...
func (cc *CustomCollector) collectBusinessMetrics() error {
	for name, metric := range cc.businessMetrics {
		prefix := fmt.Sprintf("custom.business.%s", name)
...
func (cc *CustomCollector) collectGaugeMetrics() error {
	for name, metric := range cc.gaugeMetrics {
		prefix := fmt.Sprintf("custom.gauge.%s", name)
		cc.metrics[prefix+".value"] = metric.Value
...
func (cc *CustomCollector) collectCounterMetrics() error {
	for name, metric := range cc.counterMetrics {
		prefix := fmt.Sprintf("custom.counter.%s", name)
		cc.metrics[prefix+".value"] = metric.Value
...
func (cc *CustomCollector) collectHistogramMetrics() error {
	for name, metric := range cc.histogramMetrics {
		prefix := fmt.Sprintf("custom.histogram.%s", name)
...
func (cc *CustomCollector) collectTimerMetrics() error {
	for name, metric := range cc.timerMetrics {
		prefix := fmt.Sprintf("custom.timer.%s", name)
		cc.metrics[prefix+".count"] = metric.Count
...
func (cc *CustomCollector) GetStats() map[string]interface{} {
	cc.mu.RLock()
	defer cc.mu.RUnlock()
...
func NewBusinessMetricsCollector() CustomMetricCollector {
...
func (bmc *BusinessMetricsCollector) Collect() (map[string]interface{}, error) {
	if !bmc.enabled {
		return nil, nil
	}
...
func NewFeatureUsageCollector() CustomMetricCollector {
...
func (fuc *FeatureUsageCollector) Collect() (map[string]interface{}, error) {
	if !fuc.enabled {
		return nil, nil
	}
...
func (cc *CustomCollector) CreateMetricsWrapper(metricsCollector metrics.MetricsCollector) *CustomMetricsWrapper {
