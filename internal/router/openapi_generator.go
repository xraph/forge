...
func applyForgeExtensions(op *Operation, metadata map[string]any) {
	for key, value := range ClientExtensions(metadata) {
...
// ClientExtensions translates the forge.client.* keys a route's options wrote
// into the x-forge-* extensions the client generator reads, keyed exactly as
// they appear on an OpenAPI operation. Nil when the route declared nothing.
//
// Exported because the client generator has a second way in: a router with no
// OpenAPI document is introspected from its raw route table, and that path
// used to copy the metadata verbatim and resolve none of it, so a route that
// declared WithInvalidates on a server without OpenAPI generated a client with
// an empty contract. One translation, used by both the document and the
// fallback, is what keeps the two from saying different things about the same
// route.
func ClientExtensions(metadata map[string]any) map[string]any {
	if metadata == nil {
		return nil
	}
