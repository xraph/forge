...
type AsyncHealthCheck struct {
	*BaseHealthCheck
	...
	running   bool
	stopCh    chan struct{}
	resultCh  chan *shared.HealthResult
	mu        sync.RWMutex
}
...
func NewAsyncHealthCheck(config *HealthCheckConfig, checkFunc HealthCheckFunc) *AsyncHealthCheck {
	return &AsyncHealthCheck{
		...
		resultCh:        make(chan *shared.HealthResult, 1),
	}
}
...
func (ahc *AsyncHealthCheck) Check(ctx context.Context) *shared.HealthResult {
	...
	if result == nil {
		return shared.NewHealthResult(ahc.name, shared.HealthStatusUnknown, "async health check not started")
	}

	// Check if result is stale
	if time.Since(result.Timestamp) > ahc.interval*2 {
		return shared.NewHealthResult(ahc.name, shared.HealthStatusUnhealthy, "async health check result is stale")
	}
...
func (chc *CompositeHealthCheck) Check(ctx context.Context) *shared.HealthResult {
	...
	results := make([]*shared.HealthResult, len(chc.checks))
...
func (chc *CompositeHealthCheck) aggregateResults(results []*shared.HealthResult) *shared.HealthResult {
	...
	if criticalFailed {
		overallStatus = shared.HealthStatusUnhealthy
	}
