...
// =============================================================================
// PROMETHEUS EXPORTER (legacy hand-rolled serializer)
// =============================================================================

// PrometheusExporter exports metrics in Prometheus text format without
// using client_golang. Kept for backward compatibility with the metrics
// collector pipeline; the new PrometheusBridge (prometheus.go) is preferred
// for scrape-based exposition.
type PrometheusExporter struct {
	mu        sync.Mutex
	namespace string
	subsystem string
	labels    map[string]string
	stats     *prometheusExporterStats
}

// prometheusExporterStats contains statistics about the Prometheus exporter.
type prometheusExporterStats struct {
...
// legacyPrometheusConfig contains configuration for the legacy Prometheus exporter.
type legacyPrometheusConfig struct {
...
// defaultLegacyPrometheusConfig returns default legacy Prometheus configuration.
func defaultLegacyPrometheusConfig() *legacyPrometheusConfig {
	return &legacyPrometheusConfig{
		Namespace:       "forge",
		Subsystem:       "",
...
// NewPrometheusExporter creates a new legacy Prometheus exporter.
func NewPrometheusExporter() shared.Exporter {
	return newPrometheusExporterWithConfig(defaultLegacyPrometheusConfig())
}

// newPrometheusExporterWithConfig creates a new legacy Prometheus exporter with configuration.
func newPrometheusExporterWithConfig(config *legacyPrometheusConfig) shared.Exporter {
...
func (pe *PrometheusExporter) Export(metricMap map[string]any) ([]byte, error) {
	pe.mu.Lock()
	defer pe.mu.Unlock()
	...
	groupedMetrics := pe.groupMetrics(metricMap)
...
func (pe *PrometheusExporter) groupMetrics(metricMap map[string]any) map[string][]metricEntry {
	grouped := make(map[string][]metricEntry)

	for fullName, value := range metricMap {
