...
func NewHealthMetricsCollector(store HealthStore, metrics common.Metrics, logger common.Logger, config *MetricsCollectorConfig) *HealthMetricsCollector {
	if config == nil {
...
func (hmc *HealthMetricsCollector) Start(ctx context.Context) error {
	...
	if hmc.started {
		return common.ErrServiceAlreadyExists("health-metrics-collector")
	}
...
func NewHealthMetricsExporter(collector *HealthMetricsCollector, config *ExporterConfig, logger common.Logger) *HealthMetricsExporter {
...
func NewHealthMetricsService(store HealthStore, metrics common.Metrics, logger common.Logger, config *HealthMetricsServiceConfig) *HealthMetricsService {
	if config == nil {
		config = &HealthMetricsServiceConfig{
...
func (hms *HealthMetricsService) Dependencies() []string {
	return []string{common.HealthCheckerKey, common.MetricsCollectorKey}
}
...
func (hms *HealthMetricsService) OnHealthCheck(ctx context.Context) error {
	...
	if !hms.collector.IsStarted() {
		return fmt.Errorf("health metrics collector not started")
	}
	...
	if time.Since(lastCollect) > hms.config.CollectorConfig.CollectionInterval*2 {
		return fmt.Errorf("health metrics collection is stale")
	}
