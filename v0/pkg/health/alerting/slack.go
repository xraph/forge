...
func NewSlackNotifier(name string, config *SlackConfig, logger common.Logger, metrics common.Metrics) *SlackNotifier {
	if config == nil {
...
func (snb *SlackNotifierBuilder) WithLogger(logger common.Logger) *SlackNotifierBuilder {
	snb.logger = logger
	return snb
}

// WithMetrics sets the metrics collector
func (snb *SlackNotifierBuilder) WithMetrics(metrics common.Metrics) *SlackNotifierBuilder {
	snb.metrics = metrics
	return snb
}
...
func NewSlackWebhookNotifier(name, webhookURL string, logger common.Logger, metrics common.Metrics) *SlackNotifier {
...
func NewSlackBotNotifier(name, botToken, channel string, logger common.Logger, metrics common.Metrics) *SlackNotifier {
	return NewSlackNotifierBuilder(name).
...
func NewSlackNotifierWithMentions(name, webhookURL string, mentionUsers, mentionChannels []string, logger common.Logger, metrics common.Metrics) *SlackNotifier {
	return NewSlackNotifierBuilder(name).
		WithWebhookURL(webhookURL).
