...
func (ps *PluginService) Start(ctx context.Context) error {
	if ps.started {
	...
	if err := ps.manager.Start(ctx); err != nil {
...
func (ps *PluginService) Stop(ctx context.Context) error {
	if !ps.started {
		return common.ErrServiceNotFound(ps.Name())
	}

	// Stop plugin manager
	if err := ps.manager.Stop(ctx); err != nil {
