...
`useQuery` hands back a `refetch` for the query that component opened. When the
write lives somewhere else, use `useInvalidate`:

```tsx
import { useInvalidate, useMutation } from '@forge-go/client-react';
import { useOrderArchive, useOrderList } from './generated/hooks';

function ArchiveDialog({ id, onDone }: { id: number; onDone: () => void }) {
  ...
  return (
    <button
      onClick={async () => {
        await archive.mutateAsync({ path: { id } });
...
when some other component opened it. One wrinkle. `invalidate(op)` and
`invalidate(op, {})` do not name the same query, because a read called with no
arguments keys differently from one called with an empty object, and the
...
`useInvalidate(cache)` takes the same override `useClient` takes, and resolves
the same way without one, so you invalidate through the cache you read from. The
function it hands back is stable for as long as that cache is, so you can put it
in a dependency array or hand it to a `memo`'d child.
