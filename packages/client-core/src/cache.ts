...
export class QueryCache {
  ...
  private start(record: Record_): Promise<unknown> {
    ...
    const sequence = async (): Promise<unknown> => {
      ...
      for (;;) {
        ...
        // TWO stamps, from two different clocks, answering two different
        // questions. Keeping only one silently reintroduces the bug the other
        // was added to fix.
        //
        ...
        //
        // `rootTypeOf`, not `record.meta.entity`. This line merged CLEANLY and
        // wrong: the root-type fix landed on a `store.write` call that this
        // branch replaced wholesale with `store.stage`, so git had no conflict
        // to raise and took this side entire, silently reverting an enveloped
        // response to normalizing nothing on the query path.
   ...
   * Commit a response.
   *
   * `startedAt` is the registry's clock reading from when this attempt was
   ...
   *
   * `staged` is the response already normalized but not yet committed, and
   * `skip` the entity keys a stream frame overtook while it was in flight.
   * Those are the store's frame clock rather than the registry's -- a different
   * question, answered separately; see the caller.
   */
  private settle(
    record: Record_,
    response: unknown,
