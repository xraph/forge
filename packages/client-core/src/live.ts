...
function identify(cache: QueryCache, type: string, payload: unknown): EntityKey | undefined {
  ...
  // A row with no `idField` is the same case this function's third refusal
  // already covers: a typename that cannot be keyed. Envelopes and the
  // non-entity hops between entities carry `fields` and no identity, so they
  // are walked and never stored -- and reading `payload[meta.idField]`
  // unguarded would look up the literal property "undefined" and evict under
  // `Type:undefined`, which is the guessed key this refuses to delete by.
  const idField = cache.entities[type]?.idField;

  if (idField === undefined || payload === null || typeof payload !== 'object') return undefined;
