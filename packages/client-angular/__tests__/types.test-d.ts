...
import { injectMutation } from '../src/injectMutation';
...
export function entityTypeReachesTheOptimisticOption(): void {
  const update = injectMutation(updateOrder);
  ...
  // Binding-level options are checked too, whether written as a plain object
  // or as the getter a signal-backed option list uses.
  // @ts-expect-error `stauts` is not a field of Order
  injectMutation(updateOrder, { optimistic: { stauts: 'shipped' } });
...
export function anUntypedBindingIsUnaffected(): void {
  void injectMutation(untypedUpdate).mutate({}, { optimistic: { whatever: true } });
}
