 ...
 * everyone who followed their instincts got the least. So the full panel
 * answers to `/overlay` now and this answers to `/mini`, which says what it is.
 *
 * It is still worth having. Six tables and a filter box in 3.5 kB is the right
 * trade for a production-adjacent build, a demo, or anything that wants to see
...
export function mountMini(devtools: Devtools, options: MiniOptions = {}): () => void {
  ...
  if (doc === undefined) {
    throw new Error('[forge] mountMini needs a DOM; there is no document here');
  }
