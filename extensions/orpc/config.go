...
func (c *Config) Validate() error {
	...
	if c.Endpoint == "" {
		return errors.New("orpc: endpoint cannot be empty")
	}

	if c.EnableOpenRPC && c.OpenRPCEndpoint == "" {
		return errors.New("orpc: openrpc_endpoint cannot be empty when enabled")
	}

	if c.BatchLimit < 1 {
		return errors.New("orpc: batch_limit must be at least 1")
	}

	if c.MaxRequestSize < 1024 {
		return errors.New("orpc: max_request_size must be at least 1024 bytes")
	}

	if c.RequestTimeout < 1 {
		return errors.New("orpc: request_timeout must be at least 1 second")
	}
