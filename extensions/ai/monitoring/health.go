...
func (m *AIHealthMonitor) checkSystemHealth(ctx context.Context, check *AIHealthCheck) {
	...
	if getFloat64FromMap(stats, "ErrorRate") > 0.2 {
		check.Status = forge.HealthStatusDegraded
		...
		check.Status = forge.HealthStatusHealthy
...
func (m *AIHealthMonitor) calculateOverallHealthScore() float64 {
	...
	for _, check := range m.checks {
		...
		case forge.HealthStatusHealthy:
			score = 1.0
		case forge.HealthStatusDegraded:
			score = 0.5
		case forge.HealthStatusUnhealthy:
			score = 0.0
		}
...
func (m *AIHealthMonitor) generateHealthAlerts() []*HealthAlert {
	...
	for name, check := range m.checks {
		if check.Critical && check.Status == forge.HealthStatusUnhealthy {
		...
		} else if check.Status == forge.HealthStatusDegraded {
			alerts = append(alerts, &HealthAlert{
