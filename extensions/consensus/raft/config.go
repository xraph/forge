...
func (c *Config) Validate() error {
	...
	if c.ElectionTimeoutMin <= 0 {
		return fmt.Errorf("election timeout min must be positive")
	}

	if c.ElectionTimeoutMax <= c.ElectionTimeoutMin {
		return fmt.Errorf("election timeout max must be greater than min")
	}
	...
	if c.HeartbeatInterval >= c.ElectionTimeoutMin {
		return fmt.Errorf("heartbeat interval must be less than election timeout min")
	}
	...
	if c.ApplyInterval <= 0 {
		return fmt.Errorf("apply interval must be positive")
	}
	...
	if c.EnableSnapshots {
		...
		if c.SnapshotThreshold == 0 {
			return fmt.Errorf("snapshot threshold must be positive")
		}
	}
	...
	if c.ReplicationBatchSize <= 0 {
		return fmt.Errorf("replication batch size must be positive")
	}

	if c.ReplicationBatchSize > c.MaxAppendEntries {
		return fmt.Errorf("replication batch size cannot exceed max append entries")
	}
	...
	if c.MaxInflightReplications <= 0 {
		return fmt.Errorf("max inflight replications must be positive")
	}
	...
	if c.LogCompactionEnabled && c.TrailingLogs == 0 {
		return fmt.Errorf("trailing logs must be positive when log compaction is enabled")
	}

	if c.TrailingLogs >= c.MaxLogEntries {
		return fmt.Errorf("trailing logs must be less than max log entries")
	}
