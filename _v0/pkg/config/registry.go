...
import (
	...
	"github.com/xraph/forge/v0/pkg/config/sources"
...
func NewSourceRegistry(logger common.Logger) SourceRegistry {
...
func (sr *SourceRegistryImpl) RegisterSource(source ConfigSource) error {
	...
	if sourceName == "" {
		return common.ErrValidationError("source_name", fmt.Errorf("source name cannot be empty"))
	}

	// Check if source already exists
	if _, exists := sr.sources[sourceName]; exists {
		return common.ErrServiceAlreadyExists(sourceName)
	}
	...
	if sr.metrics != nil {
		sr.metrics.Counter("forge.config.sources_registered").Inc()
		sr.metrics.Gauge("forge.config.sources_count").Set(float64(len(sr.sources)))
	}
...
func (sr *SourceRegistryImpl) UnregisterSource(name string) error {
	...
	if sr.metrics != nil {
		sr.metrics.Counter("forge.config.sources_unregistered").Inc()
		sr.metrics.Gauge("forge.config.sources_count").Set(float64(len(sr.sources)))
	}
...
func (sr *SourceRegistryImpl) Clear() error {
	...
	if sr.metrics != nil {
		sr.metrics.Counter("forge.config.sources_cleared").Inc()
		sr.metrics.Gauge("forge.config.sources_count").Set(0)
	}
