...
func NewConsensusService(l common.Logger, metrics common.Metrics, config common.ConfigManager, db database.Connection) *ConsensusService {
	...
	manager := NewManager(consensusConfig, l, metrics, db)
...
func (cs *ConsensusService) GetManager() *Manager {
	return cs.manager
}
...
// ManagerDefinition returns the consensus manager service definition
func ManagerDefinition() common.ServiceDefinition {
	return common.ServiceDefinition{
		Name: "consensus-manager",
		Type: (*Manager)(nil),
		Constructor: func(service *ConsensusService) *Manager {
