...
type DatabaseMetricsMiddleware struct {
	...
	queryRowsAffected  metrics.Histogram
	queryRowsReturned  metrics.Histogram
	queriesByTable     map[string]metrics.Counter
	queriesByOperation map[string]metrics.Counter

	// Transaction metrics
	transactionsTotal    metrics.Counter
	transactionDuration  metrics.Histogram
	transactionErrors    metrics.Counter
	transactionRollbacks metrics.Counter
	transactionCommits   metrics.Counter

	// Migration metrics
	migrationsTotal   metrics.Counter
	migrationDuration metrics.Histogram
	migrationErrors   metrics.Counter
}
...
func NewDatabaseMetricsMiddleware(collector metrics.MetricsCollector) *DatabaseMetricsMiddleware {
...
func NewDatabaseMetricsMiddlewareWithConfig(collector metrics.MetricsCollector, config *DatabaseMetricsConfig) *DatabaseMetricsMiddleware {
	return &DatabaseMetricsMiddleware{
		name:               "database-metrics",
		collector:          collector,
		config:             config,
		queriesByTable:     make(map[string]metrics.Counter),
		queriesByOperation: make(map[string]metrics.Counter),
	}
}
...
func CreateDatabaseMetricsMiddleware(collector metrics.MetricsCollector) *DatabaseMetricsMiddleware {
...
func CreateDatabaseMetricsMiddlewareWithConfig(collector metrics.MetricsCollector, config *DatabaseMetricsConfig) *DatabaseMetricsMiddleware {
