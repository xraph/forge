...
func NewWebhookNotifier(name string, config *WebhookConfig, logger common.Logger, metrics common.Metrics) *WebhookNotifier {
	if config == nil {
...
func (wnb *WebhookNotifierBuilder) WithLogger(logger common.Logger) *WebhookNotifierBuilder {
	wnb.logger = logger
	return wnb
}

// WithMetrics sets the metrics collector
func (wnb *WebhookNotifierBuilder) WithMetrics(metrics common.Metrics) *WebhookNotifierBuilder {
	wnb.metrics = metrics
	return wnb
}
...
func NewPureSlackWebhookNotifier(name, webhookURL string, logger common.Logger, metrics common.Metrics) *WebhookNotifier {
	template := `{
		"text": "{{.Title}}",
		"attachments": [
			{
...
func NewDiscordWebhookNotifier(name, webhookURL string, logger common.Logger, metrics common.Metrics) *WebhookNotifier {
	template := `{
		"embeds": [
			{
				"title": "{{.Title}}",
...
func NewMSTeamsWebhookNotifier(name, webhookURL string, logger common.Logger, metrics common.Metrics) *WebhookNotifier {
	template := `{
		"@type": "MessageCard",
