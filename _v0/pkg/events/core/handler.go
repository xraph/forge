...
func (h *TypedEventHandler) Handle(ctx context.Context, event *Event) error {
	...
	if h.metrics != nil {
		...
		h.metrics.Histogram("forge.events.handler_duration", "handler", h.name).Observe(duration.Seconds())

		if err != nil {
			h.metrics.Counter("forge.events.handler_errors", "handler", h.name).Inc()
		} else {
			h.metrics.Counter("forge.events.handler_success", "handler", h.name).Inc()
		}
	}
...
func (h *TypedEventHandler) WithMetrics(metrics common.Metrics) *TypedEventHandler {
	h.metrics = metrics
	return h
}

// WithLogger sets the logger for the handler
func (h *TypedEventHandler) WithLogger(logger common.Logger) *TypedEventHandler {
	h.logger = logger
	return h
}
...
func LoggingMiddleware(l common.Logger) HandlerMiddleware {
...
func MetricsMiddleware(metrics common.Metrics) HandlerMiddleware {
	return func(next EventHandlerFunc) EventHandlerFunc {
		return func(ctx context.Context, event *Event) error {
			start := time.Now()
			...
			if err != nil {
				metrics.Counter("forge.events.handler_errors", "event_type", event.Type).Inc()
			} else {
				metrics.Counter("forge.events.handler_success", "event_type", event.Type).Inc()
			}
...
func NewHandlerRegistry(logger common.Logger, metrics common.Metrics) *HandlerRegistry {
