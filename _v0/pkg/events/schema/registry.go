...
func NewSchemaRegistry(store SchemaStore, cache SchemaCache, config *RegistryConfig, logger common.Logger, metrics common.Metrics) *SchemaRegistry {
	if config == nil {
...
func (sr *SchemaRegistry) RegisterSchema(ctx context.Context, schema *Schema) error {
	if err := sr.validateSchema(schema); err != nil {
		return common.ErrValidationError("schema", err)
	}
